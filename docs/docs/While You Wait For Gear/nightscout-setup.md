...
You do not have to enter all the information in the profile on your NightScout website if you are using OpenAPS (since OpenAPS will be providing the information for IOB and COB rather than letting NS calculate them), but you do have to fill out the `Basal Profile` and `TimeZone` at a minimum in order to have your temp basals properly display. To do this, click on the three lines in the top right-hand corner of your NightScout page. Select "Profile Editor" and enter the values so that they match those on your pump. Click `Save` (at the bottom of the page) when you have entered the information.  You will be prompted to authenticate, if it is the first time you’ve used the device to make changes in your profile.  Click on the `Authenticate` link at the bottom of the site, and enter your API_SECRET to complete the authentication.
